...
def get_stellar_magnitudes(
    ...
    **kwargs,
    ...
    if starfile[-4:] == ".vot":
        ...
        bp_mag = 5.0
        try:
            spec = webbpsf_ext.spectra.source_spectrum(
                name="Input Data & SED",
                sptype=spectral_type,
                mag_val=bp_mag,
                bp=bp_k,
                votable_file=starfile,
                **kwargs,
            )
        except:
            spec = webbpsf_ext.spectra.source_spectrum(
        ...
        spec.fit_SED(x0=[1.0], wlim=wlim, use_err=False, verbose=False)
        ...
        input_flux = u.Quantity(
            spec.sp_model.flux, str(spec.sp_model.fluxunits).lower()
        )
        photlam_flux = convert_flux(
            spec.sp_model.wave, input_flux, out_flux_unit="photlam"
        )
        sed = SourceSpectrum(
            Empirical1D,
            points=spec.sp_model.wave << u.Unit(str(spec.sp_model.waveunits)),
    ...
    try:
        ...
        log.warning(
            "Using SVO Filter Profile Service timed out. Using cached data instead."
        )
    ...
    for i, filt in enumerate(filts):
        ...
        try:
            with importlib.resources.open_text(
                f"spaceKLIP.resources.PCEs.{instrume}", f"{filt}.txt"
            ) as bandpass_file:
        ...
        bandpass = SpectralElement(
            Empirical1D, points=bandpass_wave, lookup_table=bandpass_throughput
        )
