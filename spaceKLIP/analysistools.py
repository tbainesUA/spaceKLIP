...
from scipy.ndimage import convolve, fourier_shift, gaussian_filter, rotate
...
from spaceKLIP.psf import JWST_PSF, get_offsetpsf
...
from spaceKLIP.utils import pop_pxar_kw, set_surrounded_pixels, write_starfile
...
class AnalysisTools:
    ...
    def raw_contrast(
        ...
        **kwargs,
        ...
        print("starfile        =", repr(starfile))
        print("starfile (new)  =", repr(new_starfile_path))
        ...
        for i, key in enumerate(self.database.red.keys()):
            ...
            for j in range(nfitsfiles):
                ...
                mstar, fzero, fzero_flam, fzero_wm2um = get_stellar_magnitudes(
                    ...
                    **kwargs,
                )  # vegamag, Jy, erg/s/cm^2/A, W/m^2/um
                ...
                ax.set_title(
                    f"Masked data in {filt}, {psfsub_strategy} ({klmodes[-1]} KL)"
                )
                for r in [5, 10]:
                    ax.add_patch(
                ...
                plt.colorbar(
                    mappable=ax.images[0], label=self.database.red[key]["BUNIT"][j]
                )
                plt.tight_layout()
                if save_figures:
                ...
                for k in range(data.shape[0]):
                    if mask is None:
                        ...
                        ax.plot(
                            seps[k], cons[k], color=colors[k % mod], alpha=0.3, ls="--"
                        )
                        ax.plot(
                            seps[k],
                            cons_mask[k],
        ...
        **kwargs,
        ...
        for i, key in enumerate(self.database.red.keys()):
            ...
            for j in range(nfitsfiles):
                ...
                klip_args["numbasis"] = [
                    int(nb) for nb in self.database.red[key]["KLMODES"][j].split(",")
                ]
                klip_args["algo"] = (
                    "klip"  # Currently not logged, may need changing in future.
                )
                _, _, maxnumbasis = get_pyklip_filepaths(
                    self.database, key, return_maxbasis=True
                ...
                if use_saved:
                    log.info(
                        "Retrieving saved companion injection and recovery results."
                    )
                    ...
                    if "binarity" in kwargs.keys():
                        ...
                        output_dir_extraction = os.path.join(
                            os.path.join(
                                self.database.output_dir, kwargs["extraction_subdir"]
                            ),
                            "KL%.0f" % kwargs["klmode"],
                        )
                        output_dir_comp = os.path.join(
                            output_dir_extraction, "C%.0f" % kwargs["C"]
                        )
                        output_dir_fm = os.path.join(output_dir_comp, "KLIP_FM")

                        kwargs_inj["psf_filename"] = os.path.join(
                            output_dir_fm,
                            "FM-"
                            + klip_args["mode"]
                            + "_NANNU"
                            ...
                            + "_"
                            + key
                            + "-psf-KLmodes-all.fits",
                        )
                        kwargs_inj["companion_filename"] = os.path.join(
                            output_dir_comp,
                            klip_args["mode"]
                            + "_NANNU"
                            + str(klip_args["annuli"])
                            ...
                            + "_"
                            + key
                            + "-results_c%.0f" % (kwargs["C"])
                            + ".ecsv",
                        )
                        tab = Table.read(
                            kwargs_inj["companion_filename"], format="ascii"
                        )
                        kwargs_inj["RA"] = tab[-1]["RA"]  # arcsec
                        kwargs_inj["DEC"] = tab[-1]["DEC"]  # arcsec
                        kwargs_inj["CON"] = tab[-1]["CON"]
                        pa = np.rad2deg(
                            np.arctan2(kwargs_inj["RA"], kwargs_inj["DEC"])
                        )  # deg
                        kwargs_inj["thetas"] = [
                            pa + 90.0 - all_pa for all_pa in all_pas
                        ]
                    else:
                ...
                for si, seps in enumerate(rawseps):
                    ...
                    if mask is not None:
                        ...
                        ax.plot(
                            seps, rawcons_corr[si], alpha=0.3, ls="--", color=f"C{si}"
                        )
                        ax.legend(
                            loc="upper right",
                            ncols=3,
                ...
                for si, seps in enumerate(rawseps):
                    ...
                    if mask is not None:
                        ...
                        ax.plot(seps, rawcons[si], alpha=0.3, ls=":", color=f"C{si}")
                ax.legend(
                    loc="upper right",
                    ncols=3,
                    fontsize=10,
    ...
    def extract_companions(
        ...
        **kwargs,
        ...
        def plot_traces(
            fma, nburn=100, labels=[r"x", r"y", r"$\alpha$", "l"], path="/.traces.pdf"
        ):
            ...
            for j in range(nfitsfiles):
                ...
                if self.database.red[key]["TELESCOP"][j] == "JWST":
                    if inst == "NIRCAM":
                        ...
                        raise UserWarning(
                            "Data originates from unknown JWST instrument"
                        )
                else:
                    raise UserWarning("Data originates from unknown telescope")
                if planetfile is None:
                    ...
                    # the kwargs.
                    if (
                        "planetfile" in kwargs.keys()
                        and kwargs["planetfile"] is not None
                    ):
                ...
                if date is not None:
                    if date == "auto":
                        date = fits.getheader(
                            self.database.obs[key]["FITSFILE"][ww_sci[0]], 0
                        )["DATE-BEG"]
                if not use_epsf:
                    ...
                    epsf_builder = pupsf.EPSFBuilder(
                        oversampling=3, maxiters=50, progress_bar=True
                    )
                ...
                if split_fit:
                    if (
                        "sigma_xguess" in kwargs.keys()
                        and kwargs["sigma_xguess"] is not None
                    ):
                        sigma_xguess = kwargs["sigma_xguess"]
                    else:
                        sigma_xguess = 0.1
                    if (
                        "sigma_yguess" in kwargs.keys()
                        and kwargs["sigma_yguess"] is not None
                    ):
                        sigma_yguess = kwargs["sigma_yguess"]
                    else:
                        sigma_yguess = 0.1
                    if (
                        "theta_guess" in kwargs.keys()
                        and kwargs["theta_guess"] is not None
                    ):
                        theta_guess = kwargs["theta_guess"]
                    else:
                        theta_guess = 0
                    if (
                        "scale_guess" in kwargs.keys()
                        and kwargs["scale_guess"] is not None
                    ):
                    ...
                    if (
                        "sigma_xrange" in kwargs.keys()
                        and kwargs["sigma_xrange"] is not None
                    ):
                        sigma_xrange = kwargs["sigma_xrange"]
                    else:
                        sigma_xrange = (0.001, 5)
                    if (
                        "sigma_yrange" in kwargs.keys()
                        and kwargs["sigma_yrange"] is not None
                    ):
                        sigma_yrange = kwargs["sigma_yrange"]
                    else:
                        sigma_yrange = (0.001, 5)
                    if (
                        "theta_range" in kwargs.keys()
                        and kwargs["theta_range"] is not None
                    ):
                        theta_range = kwargs["theta_range"]
                    else:
                        theta_range = (-180, 180)
                    if (
                        "scale_range" in kwargs.keys()
                        and kwargs["scale_range"] is not None
                    ):
                ...
                for k in range(len(companions)):
                    ...
                    for ww in ww_sci:
                        ...
                        _, _, _, _, _, _, _, _, _, _, maskoffs = ut.read_obs(
                            self.database.obs[key]["FITSFILE"][ww]
                        )
                        ...
                        if maskoffs is not None:
                            ...
                            sim_sep = (
                                np.sqrt(sim_dx**2 + sim_dy**2) * pxsc_arcsec
                            )  # arcsec
                        ...
                        if offsetpsf_func.image_mask is not None:
                            ...
                            # coronagraphic mask.
                            scale_factor = np.sum(offsetpsf_coronmsk) / np.sum(
                                psf_no_coronmsk
                            )
                        ...
                        if not use_epsf:
                            ...
                            offsetpsf = offsetpsf_func.evaluate(
                                x, y, 1, fov_pix // 2, fov_pix // 2
                            )
                        offsetpsf *= (
                        ...
                        if not np.isnan(self.database.obs[key]["BLURFWHM"][ww]):
                            gauss_sigma = self.database.obs[key]["BLURFWHM"][
                                j
                    ...
                    if inject == False:
                        ...
                        if remove_background:
                            ...
                            corr_len_label = r"$l$"
                            fma.set_kernel(
                                fitkernel, [corr_len_guess], [corr_len_label]
                            )
                            ...
                            if "nburn" not in kwargs.keys() or kwargs["nburn"] is None:
                                ...
                                nburn = kwargs["nburn"]
                            if (
                                "nsteps" not in kwargs.keys()
                                or kwargs["nsteps"] is None
                            ):
                                nsteps = 100
                            else:
                            ...
                            stamp = stamp[
                                yp - hsz : yp + hsz + 1, xp - hsz : xp + hsz + 1
                            ]
                            psf = fm_frame.copy()
                        ...
                        if fitmethod == "mcmc":
                            ...
                            rel_err = np.sqrt(
                                (fma.fit_flux.error / fma.fit_flux.bestfit) ** 2
                                + (mstar_err_temp * np.log(10) / 2.5) ** 2
                            )
                            flux_jy = (
                            ...
                            fstar_jy_err = fstar_jy * (
                                mstar_err_temp * np.log(10) / 2.5
                            )  # Jy
                            fstar_flam = fzero_flam[filt] / 10 ** (
                                mstar[filt] / 2.5
                            )  # erg/s/cm^2/A
                            fstar_flam_err = fstar_flam * (
                                mstar_err_temp * np.log(10) / 2.5
                            )  # erg/s/cm^2/A
                            fstar_wm2um = fzero_wm2um[filt] / 10 ** (
                                mstar[filt] / 2.5
                            )  # W/m^2/um
                            fstar_wm2um_err = fstar_wm2um * (
                                mstar_err_temp * np.log(10) / 2.5
                            )  # W/m^2/um
                            delmag = -2.5 * np.log10(
                            ...
                            if split_fit:
                                ...
                                tab.add_row(
                                    (
                                        k + 1,
                                        fma.raw_RA_offset.bestfit
                                        * pxsc_arcsec,  # arcsec
                                        fma.raw_RA_offset.error * pxsc_arcsec,  # arcsec
                                        fma.raw_Dec_offset.bestfit
                                        * pxsc_arcsec,  # arcsec
                                        fma.raw_Dec_offset.error
                                        * pxsc_arcsec,  # arcsec
                                        flux_jy,
                                        flux_jy_err,
                                        flux_flam,
                        ...
                        # Nested sampling.
                            ...
                            output_dir_ns = os.path.join(
                                output_dir_comp, "temp-multinest/"
                            )
                            ...
                            fit.generate_fm_stamp(
                                fm_frame, [fm_centx, fm_centy], padding=5
                            )
                            ...
                            if isinstance(mstar_err, dict):
                                ...
                                mstar_err_temp = mstar_err
                            rel_err = np.sqrt(
                                (fit.fit_flux.error / fit.fit_flux.bestfit) ** 2
                                + (mstar_err_temp * np.log(10) / 2.5) ** 2
                            )
                            flux_jy = (
                        ...
                        if remove_background:
                            ...
                            p1 = ax[1].imshow(
                                np.abs(psf_shift) < thresh, origin="lower"
                            )
                    ...
                    if subtract or inject:
                        ...
                        if save_preklip:
                            ...
                            for l in range(len(self.database.obs[key])):
                                file = os.path.split(
                                    self.database.obs[key]["FITSFILE"][l]
                                )[1]
                                self.database.obs[key]["FITSFILE"][l] = os.path.join(
                                    output_dir_pk, file
                                )
                            file = os.path.split(self.database.red[key]["FITSFILE"][j])[
                                1
                            ]
                            file = file[file.find("JWST") : file.find("-KLmodes-all")]
                            file = os.path.join(output_dir_fm, file + ".dat")
                            self.database.obs[key].write(
                        ...
                        temp = os.path.join(
                            output_dir_fm, fileprefix + "-KLmodes-all.fits"
                        )
                        hdul = fits.open(temp)
                        hdul[0].header = head
    ...
    **kwargs,
    ...
    if len(list_of_injected) != 0:
        if len(list_of_injected) < Nsep * Npa:
            ...
            raise ValueError(
                "--> All {} source positions not suitable for injection.".format(
                    Nsep * Npa
                )
            )
    else:
